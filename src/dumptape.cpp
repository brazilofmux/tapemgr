...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        if (!tapeDumper.scanTape()) {
            std::cerr << "Error: No valid files found on tape." << std::endl;
            return 1;
        }
        ...
        // Temporarily keep the old implementation for comparison
        if (options.verbosity >= VerbosityLevel::Debug) {
            std::cout << "\nRunning original implementation for comparison:" << std::endl;
            processTape(options.inputFiles[0], options.verbosity);
        }
