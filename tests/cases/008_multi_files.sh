...
cat > "/data/tests/data/file1.txt" << EOF
...
echo "# Note: When extracted, trailing spaces will be trimmed from fixed-length text records" > "/data/tests/data/trimming_note.txt"

# File 2: Variable format (V)
cat > "/data/tests/data/file2.txt" << EOF
...
for line in lines:
    print(line)" > "/data/tests/data/file3.txt"
  ...
  "files": [
    {
      ...
      "local_file": "/data/tests/data/file1.txt",
      "record_format": "FB",
      "record_length": 60,
      "block_size": 300
    },
    {
      ...
      "local_file": "/data/tests/data/file2.txt",
      "record_format": "V",
      "record_length": 100,
      "block_size": 400
    },
    {
      ...
      "local_file": "/data/tests/data/file3.txt",
      "record_format": "VS",
      "record_length": 250,
      "block_size": 100
    }
...
cat > "/data/tests/config/multi_extract.json" << EOF
{
  ...
  "files": [
    {
      "dataset_name": "TEST.FILE1",
      "local_file": "/data/tests/output/file1_output.txt",
      "record_format": "FB",
      "record_length": 60,
      "block_size": 300
    },
    {
      "dataset_name": "TEST.FILE2",
      "local_file": "/data/tests/output/file2_output.txt",
      "record_format": "V",
      "record_length": 100,
      "block_size": 400
    },
    {
      "dataset_name": "TEST.FILE3",
      "local_file": "/data/tests/output/file3_output.txt",
      "record_format": "VS",
      "record_length": 250,
      "block_size": 100
    }
...
/app/tapemgr create --volser=TEST01 -o /data/tests/output/multi_files.aws -c /data/tests/config/multi_create.json

echo "Scanning tape to verify multiple files..."
/app/tapemgr scan /data/tests/output/multi_files.aws > /data/tests/output/scan_results.txt

# Verify correct number of files in scan output
file_count=$(grep -c "Dataset:" /data/tests/output/scan_results.txt)
...
/app/tapemgr extract -c /data/tests/config/multi_extract.json /data/tests/output/multi_files.aws
...
for file_num in 1 2 3; do
    ...
    if [ $file_num -eq 1 ]; then
        ...
        if diff <(sed 's/ *$//' "/data/tests/data/file${file_num}.txt") "/data/tests/output/file${file_num}_output.txt" > /dev/null; then
            ...
            diff <(sed 's/ *$//' "/data/tests/data/file${file_num}.txt") "/data/tests/output/file${file_num}_output.txt"
            failures=$((failures + 1))
        fi
    else
        # For other files, compare normally
        if diff "/data/tests/data/file${file_num}.txt" "/data/tests/output/file${file_num}_output.txt" > /dev/null; then
            echo "✓ File $file_num extraction test passed: Files match"
        else
            echo "✗ File $file_num extraction test failed: Files differ"
            diff "/data/tests/data/file${file_num}.txt" "/data/tests/output/file${file_num}_output.txt"
            failures=$((failures + 1))
        fi
    fi
done
...
/app/tapemgr init -o /data/tests/output/auto_config.json /data/tests/output/multi_files.aws

# Verify JSON config contains all three files
file_entries=$(grep -c "dataset_name" /data/tests/output/auto_config.json)
